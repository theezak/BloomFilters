...
namespace TBag.BloomFilter.Test.Standard
{
    ...
    public class CompressTest
    {
        ...
        public void StandardCompressTest()
        {
            ...
            var filter = new BloomFilter<long>(new DefaultBloomFilterConfiguration());
            ...
            var basecount = DataGenerator
                ...
                .Skip(addSize)
                .Take(addSize)
                .Count(itm => filter.Contains(itm.Id));
           ...
           Assert.AreEqual(filter.Capacity, 500000, "Unexpected size of reverse Bloom filter.");
            ...
            //check error rate.
            var notFoundCount = DataGenerator.Generate().Skip(addSize).Take(addSize).Count(itm => filter.Contains(itm.Id));
            Assert.IsTrue(notFoundCount <= basecount, "Uncompressed Bloom filter exceeded error rate.");
            Assert.IsTrue(data.All(d => filter.Contains(d.Id)), "False negatives found in uncompressed filter");
            filter = filter.Compress(true);
            Assert.AreEqual(filter.Capacity, 21739, "Unexpected size of compressed Bloom filter.");
            var compressNotFoundCount = DataGenerator.Generate().Skip(addSize).Take(addSize).Count(itm => filter.Contains(itm.Id));
            Assert.IsTrue(data.All(d => filter.Contains(d.Id)), "False negatives found in compressed filter");
